...
function wsmValidateDateTime($dateStr, $format='Y-m-d'){ 
	...
	try{
		...
		echo sprintf( '<p>'. __( 'Message: This timezone(%s) is not supported in this PHP version %s.', 'wsm' ) .'</p>', $timezone, phpversion() );
	}
}
...
function wsmGetTimezoneString() {
    global $arrCashedStats;
    if(isset($arrCashedStats['timeZone']) && $arrCashedStats['timeZone']!=''){
...
function wsmFnGetLocationInfo($ip){        
    ...
    if(is_object($infoResult)){
        if($infoResult->geoplugin_status=='404'){
            return null;
        }
    }
    return $infoResult;
...
function wsmFnGetMonthDropDown($name,$selected='',$selectedYear=''){
     ...
     $arrMonths=array('01'=>__('January',WSM_PREFIX),'02'=>__('February',WSM_PREFIX),'03'=>__('March',WSM_PREFIX),'04'=>__('April',WSM_PREFIX),'05'=>__('May',WSM_PREFIX),'06'=>__('June',WSM_PREFIX),'07'=>__('July',WSM_PREFIX),'08'=>__('August',WSM_PREFIX),'09'=>__('September',WSM_PREFIX),'10'=>__('October',WSM_PREFIX),'11'=>__('November',WSM_PREFIX),'12'=>__('December',WSM_PREFIX));
     $index=0;    
...
function wsmFnGetPagination($totalRecords, $currentPage, $url, $limit = false ){
    ...
    if ($currentPage > 1) {
        $html.='<li><a href="'.$url.'&wsmp='.($currentPage-1).'">'.__('Previous',WSM_PREFIX).'</a></li>';
    }    
    for($i=$startPage; $i<=$endPage; $i++){
        $html.='<li>';
        if($i==$currentPage){            
            $html.='<span class="wsmCurrent">'.__($i,WSM_PREFIX).'</span>';
        }else{
            $html.='<a href="'.$url.'&wsmp='.$i.'">'.__($i,WSM_PREFIX).'</a>';
        }
        $html.='</li>';
    }
    if ($currentPage < $endPage) {
        $html.='<li><a href="'.$url.'&wsmp='.($currentPage+1).'">'.__('Next',WSM_PREFIX).'</a></li>';
    }    
    $html.='</ul>';
    return $html;
}
